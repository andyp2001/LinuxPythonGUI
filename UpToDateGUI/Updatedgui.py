...
class MyWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        button = QPushButton('Stress Test', self)
        button.move(145, 350)
        button.setStyleSheet("background-color: #e8e8e8; border: 2px solid white;")
        button.setFixedSize(110, 30)
        button.clicked.connect(self.memory_stress_test)
    ...
    def Sender_application(self):
        ...
        if match:
            ...
            msg_box.information(self, 'Current IP Address',
                                f'<span style="color:white">The current IP address is: {ip}</span>')
    ...
    def memory_stress_test(self):
        ...
        # Set up progress dialog
        progress = QProgressDialog('<span style="color:white">Running memory stress test...</span>', 'Cancel', 0, 0,
                                   self)
        progress.setWindowTitle('Memory Stress Test')
        progress.setWindowModality(Qt.WindowModal)
        progress.setMinimumDuration(0)
        progress.show()
        app.processEvents()
        ...
        try:
            subprocess.check_call(f'stress --vm-bytes {mem_avail}M --vm-keep -m 1', shell=True)
        except subprocess.CalledProcessError as e:
            progress.close()
            QMessageBox.critical(self, 'Error',
                                 f'<span style="color:white">Memory stress test failed with error code {e.returncode}</span>',
                                 QMessageBox.Ok)
            return

        progress.close()
        msg_box = QMessageBox()
        msg_box.information(self, 'Memory Stress Test',
                            f'<span style="color:white">Memory stress test completed successfully.\n\nMemory usage:\n{mem_used}MB used\n{mem_avail}MB available\n{mem_total}MB total\n{mem_percent}% used</span>')
        msg_box.setStyleSheet("QLabel{min-width: 300px;}")
